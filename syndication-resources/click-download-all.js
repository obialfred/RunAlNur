// Click-based download script - bypasses CORS by using native link clicks
// This will download all 114 files to your default Downloads folder
...
// IMPORTANT: Before running, make sure your Chrome settings have:
// Settings > Downloads > "Ask where to save each file before downloading" = OFF
...
function downloadNext() {
  if (index >= downloadUrls.length) {
    console.log(`\n✅ ALL DONE! Triggered ${downloaded} downloads.`);
    console.log('Check your Downloads folder - files should be downloading.');
    return;
  }
  
  const {url, name} = downloadUrls[index];
  console.log(`[${index + 1}/${downloadUrls.length}] Downloading: ${name}`);
  ...
  // Download next file after delay (500ms between files)
  setTimeout(downloadNext, 500);
}

console.log('🚀 Starting downloads...');
console.log('📁 Files will go to your default Downloads folder');
console.log('⚠️  Make sure "Ask where to save" is OFF in Chrome settings!\n');
